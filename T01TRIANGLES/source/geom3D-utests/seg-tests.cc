...
TEST(SegTests, ValidationTest)
{
    for (int i = 0; i < 6; ++i)
    {
        fp_t withNan[6] = {};
        withNan[i] = nan;
        Segment nanSeg{Point{withNan[0], withNan[1], withNan[2]}, Point{withNan[3], withNan[4], withNan[5]}};
        fp_t withInf[6] = {};
        withInf[i] = inf;
        Segment infSeg{Point{withInf[0], withInf[1], withInf[2]}, Point{withInf[3], withInf[4], withInf[5]}};
