...
template <class T> MathVector<T> genMathVec(size_t size = DEFAULT_GEN_SIZE_)
{
...
template <class T>
SquareMatrix<T> genUpperTriangularMatirx(const std::vector<T> &diagElems = std::vector<T>(DEFAULT_GEN_SIZE_, 1))
{
...
template <class T> SquareMatrix<T> &shuffleSquareMatCols(SquareMatrix<T> &toShuffle)
{
    ...
    for (size_t i = 0; i < matSize; ++i)
    {
        size_t id1 = std::rand() % matSize, id2 = std::rand() % matSize;
        if (id1 != id2)
        {
